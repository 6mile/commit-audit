...
if [[ $REMOTEGIT == "1" ]]; then
		...
		if [[ $CSV != "1" ]]; then echo "${mag}ATTENTION: Remote Git repos will never show as ${grn}TRUE-VERIFIED${mag} unless you import their keys.${end}";fi
...
GITARY=$(git log --pretty='format:%H|%aN|%s|%G?|%ae')
#GITARY2=$(git log --pretty='format:%H|%aN|%s|%G?|%ae' | grep -v "users.noreply.github.com")
...
GHSIGS=$(echo "$GITARY" | grep "users.noreply.github.com" | wc -l | xargs)
...
GHNAMES=$(echo "$GITARY" | grep "users.noreply.github.com")
...
PERCENTGITHUB=$(bc <<< "scale=4; ($GHSIGS/$NUMBERCOMMITS) * 100" | sed '/\./ s/\.\{0,1\}0\{1,\}$//')


if [[ $CSV == "1" ]] && [[ $REMOTEGIT == "1" ]]; then
	echo "TARGET=\"$GITTARGET\",TRUE-VERIFIED=$PERCENT,UN-VERIFIED=$PERCENTOTHER,GITHUB-SIGNED=$PERCENTGITHUB,EXPIRED/REVOKED=$PERCENTEXPIRED,BAD=$PERCENTBAD"	
...
if [[ $DEVSTATS == "1" ]] && [[ $CSV != "1" ]]; then 
	...
	echo "${blu}NAME: ${end}$DEVCOMMITNUMBER, ${grn}TRUE-VERIFIED, ${cyn}UN-VERIFIED:, ${yel}GITHUB-SIGNED:, ${ora}EXPIRED/REVOKED:, ${red}BAD/UN-SIGNED:${end}" | awk -F',' '{ printf "%-36s %-23s %-20s %-20s %-20s %-20s %-20s\n", $1, $2, $3, $4, $5, $6, $7}'
	for devname in $(<tempfile); do
		if [[ -n $devname ]]; then
                	devname2=$(echo $devname | sed "s/\_/ /g")
                	DEVCOMMITNUMBER=$(echo "$GITARY" | grep -ic "$devname2" | xargs)
                	DEVGOODCOMMITS=$(echo "$GITARY" | grep -i "$devname2" | awk -F '|' '{ if($4 == "G"){print $1,$2,$3,$4,$5;} }' | wc -l | xargs)
                	DEVEXPIREDCOMMITS=$(echo "$GITARY" | grep -i "$devname2" | awk -F '|' '{ if($4 == "X" || $4 == "Y"){print $1,$2,$3,$4,$5;} }' | wc -l | xargs)
                	DEVGITHUBCOMMITS=$(echo "$GITARY" | grep -i "$devname2" | grep "users.noreply.github.com" | wc -l | xargs)
                	DEVBADCOMMITS=$(echo "$GITARY" | awk -F '|' '{ if($4 == "N" || $4 == "B"){print $1,$2,$3,$4,$5;} }' | grep -ic "$devname2" | xargs)
                	DEVUNKNOWNSIGS=$(echo "$GITARY" | awk -F '|' '{ if($4 == "E" || $4 == "U" || $4 == "R"){print $1,$2,$3,$4,$5;} } ' | grep -ic "$devname2" | xargs)
                	echo "${blu}$devname2, ${grn}$DEVGOODCOMMITS, ${cyn}$DEVUNKNOWNSIGS, ${yel}$DEVGITHUBCOMMITS, ${ora}$DEVEXPIREDCOMMITS, ${red}$DEVBADCOMMITS${end}"| awk -F',' '{ printf "%-36s %-23s %-20s %-24s %-20s %-20s\n", $1, $2, $3, $4, $5, $6, $7}'
        	fi
	done
fi
